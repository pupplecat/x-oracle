...
public entry fun register_supra_pair_id<CoinType>(
    ...
    assert!(
        object::id(supra_registry) == supra_registry_cap.for_registry,
...
public entry fun get_supra_pair_id<CoinType>(supra_registry: &SupraRegistry): u32 {
    let coin_type = type_name::get<CoinType>();
    let pair_id = table::borrow<TypeName, u32>(&supra_registry.table, coin_type);
    *pair_id
}

public fun assert_supra_pair_id<CoinType>(supra_registry: &SupraRegistry, pair_id: u32) {
    let coin_type = type_name::get<CoinType>();
