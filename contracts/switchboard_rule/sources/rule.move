...
public fun set_price<CoinType>(
    ...
    let (price_value, price_decimals, updated_time) = switchboard_adaptor::get_switchboard_price(
        aggregator,
    );

    let formatted_decimals: u8 = price_feed::decimals();
    let price_value_formatted = if (price_decimals < formatted_decimals) {
        price_value * (std::u64::pow(10, formatted_decimals - price_decimals) as u128)
    } else {
        price_value / (std::u64::pow(10, price_decimals - formatted_decimals) as u128)
    };
    assert!(
        price_value_formatted > 0 && price_value_formatted < U64_MAX,
        ERR_BAD_SWITCHBOARD_PRICE,
    );
